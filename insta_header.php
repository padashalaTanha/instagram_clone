...
<html lang="en">
  <head>
    ...
    <!-- <link href="emoji/vendor/emoji-picker/lib/css/emoji.css" rel="stylesheet"> -->
    <!-- <link href="emoji/style.css" rel="stylesheet"> -->

    <!-- <title>How to Insert Emoji using PHP in Comments</title>
    <script src="emoji/vendor/jquery/jquery-3.2.1.min.js"></script>
    <script src="emoji/vendor/emoji-picker/lib/js/config.js"></script>
    <script src="emoji/vendor/emoji-picker/lib/js/util.js"></script>
    <script src="emoji/vendor/emoji-picker/lib/js/jquery.emojiarea.js"></script>
    <script src="emoji/vendor/emoji-picker/lib/js/emoji-picker.js"></script> -->
    ...
    <header class="header">
      <nav class="header__content">
        ...
        <div class="header__buttons header__buttons--desktop">
          ...
          <a type="button" class="fa fa-plus text-dark" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="text-decoration:none"></a>
          ...
          <?php
              $userId = $_SESSION['admin']['id'];
                ...
                $allfollow = mysqli_fetch_array($userData);
                // $allassocData = json_decode($allfollow['followers'], true);  

                ?>

          <div class="container">

              <a href="javascript:hideShow()" class="fa fa-heart-o" style="position:relative; color:black; text-decoration:none; font-size:23px" ></a>
              <div style="position:absolute; margin-top:20px;" id="requestDataShow">
                <div  style="background-color:gray; padding-left:10px; padding-right:50px; padding-top:10px; border:2px black solid; color:white;">
                      <?php
                  $id = $_SESSION['admin']['id'];
                    if(!empty($allassocData))
                    {
                      for($i=0; $i<sizeof($allassocData); $i++){
                        $yourId = $allassocData[$i]['id'];
                        $quu = "select * from user where `id`='$yourId'";
                        $quu1 = mysqli_query($con,$quu); 
                        while($quu2 = mysqli_fetch_array($quu1)){
                           ?>
                    <div style="display:flex; margin-right:10px;">
                            <button class="profile-button">
                        <div class="profile-button__border"></div>
                        <div class="profile-button__picture">
                          <img src="assets/default-user.png" style="margin-right:10px;" alt="User Picture" />
                        </div>
                      </button>
                      <h6 style="margin-left:10px; padding-top:5px"><?php echo $quu2['username']; ?></h6>
                      <span id="followBckButton<?php echo $i; ?>" style="margin-left:10px;">
                      <?php 
                          // print_r($allassocData);

                          // print_r($quu2);
                          $quu2follow = json_decode($quu2['followers'], true);

                          if($allassocData[$i]['status']==0){ ?>
                              <div style="display:flex;">
                                <button class="btn btn-primary" style="font-size:11px; display:flex; margin-right:10px" id="confirmRequest" onclick="return confirmYourId(<?php echo $id; ?>, <?php echo $yourId;?>,<?php echo $i; ?>)">Confirm</button>
                                <button class="btn btn-dark" style="font-size:11px; display:flex;" name="delet">Delete</button>
                            </div>

                          <?php } else if($allassocData[$i]['status']==1){ 
                               if(@$quu2follow[$i]['status']==1 && $allassocData[$i]['status']==1){ ?>
                                  <button class="side-menu__suggestion-button" >Following...</button>
                              <?php } else { ?>
                              <button class="btn btn-primary" style="font-size:11px; display:flex; " id="confirmRequest" onclick="return FollowingRequest(<?php echo $id; ?>, <?php echo $yourId;?>,<?php echo $i; ?>)">Follow Back >></button>
                          <?php  } }else if(@$quu2follow[$i]['status']==0){ ?>
                          ...
                          <button class="side-menu__suggestion-button" onclick="return followRequestRemove(<?php echo $myId; ?>, <?php echo $yourId; ?>)">Request</button>
                            ...
                            <?php } else if(@$quu2follow[$i]['status']==1){ ?>
              ...
              <a href="insta_profile.php" class="fa fa-user-circle-o" style="color:gray; text-decoration:none; font-size:22px"></a>
          <div>
          <div class="dropdown">
                <a class="text-dark fa fa-gear" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false" style="font-size:25px; text-decoration:none; margin-top:3px">
